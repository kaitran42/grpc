...
// This file contains the implementation details of HTTP2 GOAWAY mechanism.
...
enum class GoawayState : uint8_t {
  ...
  kIdle,
  // A graceful GOAWAY is scheduled to be sent in the next transport write
  // cycle.
...
class GoawayManager {
 private:
  ...
  //    kGracefulGoawayScheduled or kGracefulFinalGoawayScheduled), then the
  //    immediate GOAWAY request takes precedence and the current error/debug
  //    data will be sent in the next transport write cycle. The graceful GOAWAY
  ...
  //       kGracefulFinalGoawayScheduled and a write cycle is triggered. The
  ...
  //    e. Once the state is kGracefulFinalGoawayScheduled, the transport write
  //    cycle
                          ...
                          if (ctx->goaway_state ==
                            ...
                            GRPC_HTTP2_GOAWAY_LOG
                                ...
                                << " -> "
                                   "kGracefulFinalGoawayScheduled.";
                            ctx->goaway_state =
                                GoawayState::kGracefulFinalGoawayScheduled;
                            ctx->goaway_interface->TriggerWriteCycle();
                          }
                          return Pending{};
                        });
