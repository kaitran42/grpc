...
YODEL_TEST(GoawayTest, MultipleImmediateGoawayRequests) {
  ...
  GoawayManager goaway_manager(std::move(goaway_interface));
  constexpr uint32_t kLastGoodStreamId = 123;
  constexpr absl::string_view kDebugData1 = "immediate_goaway_request2";
  ...
  SpawnGoawayRequest(goaway_manager, kImmediate, kErrorCode1, kDebugData1,
                     kLastGoodStreamId,
...
//    kGracefulGoawayScheduled -> kGracefulFinalGoawayScheduled -> kDone.
...
// GOAWAY request when the graceful GOAWAY is in the kGracefulGoawayScheduled
// state.
// The test asserts the following:
...
YODEL_TEST(GoawayTest, ImmediateGoawayTakesPrecedenceOverGracefulGoaway1) {
  constexpr uint32_t kLastGoodStreamId = 123;
  constexpr absl::string_view kDebugDataImmediate = "Immediate_goaway_request";
  constexpr http2::Http2ErrorCode kErrorCodeImmediate =
      http2::Http2ErrorCode::kProtocolError;
  constexpr absl::string_view kDebugDataGraceful = "graceful_goaway_request";
  constexpr http2::Http2ErrorCode kErrorCodeGraceful =
  ...
  SpawnGoawayRequest(goaway_manager, kImmediate, kErrorCodeGraceful,
                     kDebugDataGraceful, kLastGoodStreamId,
                     [&](absl::Status status) { on_done.Call(status); });
  GetParty()->Spawn(
      "goaway-request2",
      [&]() {
        return TrySeq(
            ...
            [&](auto) {
              ...
              return goaway_manager.RequestGoaway(
                  kErrorCodeImmediate,
                  Slice::FromCopiedString(kDebugDataImmediate),
                  kLastGoodStreamId,
                  /*immediate=*/true);
            },
            [&]() {
  ...
  GetParty()->Spawn(
      ...
      [&]() {
        ...
        Http2Frame expected_goaway_frame = Http2GoawayFrame{
            /*last_stream_id=*/RFC9113::kMaxStreamId31Bit,
            /*error_code=*/
            static_cast<uint32_t>(kErrorCodeGraceful),
            /*debug_data=*/Slice::FromCopiedString(kDebugDataGraceful)};
        EXPECT_EQ(goaway_frame, expected_goaway_frame);
        goaway_manager.NotifyGoawaySent();
        EXPECT_EQ(goaway_manager.TestOnlyGetGoawayState(),
                  http2::GoawayState::kGracefulGoawayScheduled);
      ...
      TrySeq(goaway2_ready_to_send.Wait(),
             [&]() {
               ...
               Http2Frame expected_goaway_frame = Http2GoawayFrame{
                   ...
                   /*error_code=*/static_cast<uint32_t>(kErrorCodeImmediate),
                   /*debug_data=*/Slice::FromCopiedString(kDebugDataImmediate)};
               EXPECT_EQ(goaway_frame, expected_goaway_frame);
               goaway_manager.NotifyGoawaySent();
               EXPECT_EQ(goaway_manager.TestOnlyGetGoawayState(),
                         http2::GoawayState::kDone);
...
// kGracefulFinalGoawayScheduled state. The test asserts the following:
